...
| The same authors "call for evaluations that measure which facts, uncertainties, and legitimate disagreements survive interaction" | same paper | That artifact is this repo's centerpiece: the **minority report** (disagreement metrics, contested virtues, the dissenter verbatim) plus full per-seat transcripts - and every deliberation now reports **fact survival** (which seat concerns are lexically traceable into the synthesis/answer) and **dissent engagement**, mechanically, at zero extra LLM cost |
...
| Council **procedure** decides the outcome: only 4 of 7 governance structures beat the best single model at all, and the best - **Deliberate → Vote** - gained **+9.2pp** with small open models | [llm-council-governance](https://github.com/andybhall/llm-council-governance) | Governance is explicit, named, and swappable: five formats (deliberation / socratic / oxford / delphi / **vote** - the study's winning structure, here with fully independent seats), and per-seat providers let the bench run on small local models - where ensemble gains are largest |
...
**Per-mode ablations.** Governance is a variable here, not a constant - run the same benchmark under any debate format and compare: `pnpm eval --file evals/questions.json --mode vote` (likewise `socratic`, `oxford`, `delphi`). The `vote` mode is the governance study's winning Deliberate → Vote structure adapted to independent seats: median seat score, plurality recommendation, no critic pass - one fewer LLM call per deliberation.
